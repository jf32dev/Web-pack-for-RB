...
const messages = defineMessages({
  userBadges: { id: 'user-badges', defaultMessage: 'User Badges' },
...
function mapStateToProps(state) {
  ...
  const colorList = gamification.userBadges.map(obj => obj.colour);
  const valueList = gamification.userBadges.map(obj => obj.max);
  valueList.pop();// remove 100 from array

  return {
    minimum: gamification.userBadgesMin,
    maximum: gamification.userBadgesMax,
    badgeLoaded: gamification.badgeLoaded,
    badgeLoading: gamification.badgeLoading,
    badgeError: gamification.badgeError,
    isResetLoading: gamification.isUserResetBadgeLoading,

    list: gamification.userBadges,
    colorList: colorList,
    valueList: valueList
  };
}
...
export default class AdminSocialBadges extends Component {
  static contextTypes = {
  ...
  handleRangeChange(nList) {
    this.props.setData({ userBadges: nList });
  }
  ...
  handleAddBadge(newBadge) {
    ...
    this.props.setData({ userBadges: [...nList, newBadge] });
  }
